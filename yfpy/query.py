...
class YahooFantasySportsQuery(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            yahoo_consumer_key (str, optional): User defined Yahoo developer app consumer key (must be provided
                in conjunction with yahoo_consumer_secret).
            yahoo_consumer_secret (str, optional): User defined Yahoo developer app consumer secret (must be
            ...
            env_var_fallback (bool, optional): Fall back to values retrieved from environment variables for any
                missing arguments or access token fields (defaults to True).
            env_file_location (Path, optional): Path to directory where existing .env file is located or new .env
                file should be generated when provided in conjunction with save_access_token_data_to_env_file=True
                (defaults to None).
            save_token_data_to_env_file (bool, optional): Boolean to save Yahoo access token data to local .env
                file (must be provided in conjunction with env_file_location) (defaults to False)
            all_output_as_json_str (bool, optional): Option to automatically convert all query output to JSON
                strings.
            browser_callback (bool, optional): Enable or disable (enabled by default) whether the yahoo-oauth
            ...
            backoff (int, optional): Multiplier that incrementally increases the wait time before retrying a
                failed query request.
            offline (bool, optional): Boolean to run in offline mode (Only works if all needed Yahoo Fantasy data
    ...
    def save_access_token_data_to_env_file(self, env_file_location: Path, env_file_name: str = ".env",
        ...
        Args:
            ...
            env_file_name (str, optional): The name of the target .env file (defaults to ".env").
            save_json_to_var_only (bool, optional): Boolean to determine whether or not to write a JSON string of
    ...
    def query(self, url: str, data_key_list: Union[List[str], List[List[str]]], data_type_class: Type = None,
              sort_function: Callable = None) -> (Union[str, YFO, List[YFO], Dict[str, YFO], None]):
        ...
        Args:
            ...
            data_type_class (Type, optional): Highest level data model type (if one exists for the retrieved
                data).
